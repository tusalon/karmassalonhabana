...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '8b7ed306-e548-4a12-8d26-f9fb067ba7cd'; // ID de Karma’s Salón Habana
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Karma’s Salón Habana';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '59286880';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'karmassalonhabana@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Karma’s Salón Habana!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'karmassalonhabana';
};
...
console.log('✅ config-negocio.js listo para Karma’s Salón Habana');
